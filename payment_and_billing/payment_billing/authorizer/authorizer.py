...
def lambda_handler(event, context):
    ...
    # Add billing and payment methods specific resources/methods
    policy.allow_method(HttpVerb.GET, f"/billing/{principal_id}")
    policy.allow_method(HttpVerb.PUT, f"/billing/{principal_id}")
    policy.allow_method(HttpVerb.DELETE, f"/billing/{principal_id}")
    policy.allow_method(HttpVerb.GET, f"/billing/{principal_id}/*")
    policy.allow_method(HttpVerb.PUT, f"/billing/{principal_id}/*")
    policy.allow_method(HttpVerb.DELETE, f"/billing/{principal_id}/*")
    
    policy.allow_method(HttpVerb.GET, f"/payment-methods/{principal_id}")
    policy.allow_method(HttpVerb.PUT, f"/payment-methods/{principal_id}")
    policy.allow_method(HttpVerb.DELETE, f"/payment-methods/{principal_id}")
    policy.allow_method(HttpVerb.GET, f"/payment-methods/{principal_id}/*")
    policy.allow_method(HttpVerb.PUT, f"/payment-methods/{principal_id}/*")
    policy.allow_method(HttpVerb.DELETE, f"/payment-methods/{principal_id}/*")
    ...
    if 'cognito:groups' in validated_decoded_token and validated_decoded_token['cognito:groups'][0] == admin_group_name:
        ...
        policy.allow_method(HttpVerb.GET, "billing")
        policy.allow_method(HttpVerb.GET, "billing/*")
        policy.allow_method(HttpVerb.DELETE, "billing")
        policy.allow_method(HttpVerb.DELETE, "billing/*")
        policy.allow_method(HttpVerb.POST, "billing")
        policy.allow_method(HttpVerb.PUT, "billing/*")
        
        policy.allow_method(HttpVerb.GET, "payment-methods")
        policy.allow_method(HttpVerb.GET, "payment-methods/*")
        policy.allow_method(HttpVerb.DELETE, "payment-methods")
        policy.allow_method(HttpVerb.DELETE, "payment-methods/*")
        policy.allow_method(HttpVerb.POST, "payment-methods")
        policy.allow_method(HttpVerb.PUT, "payment-methods/*")
...
class AuthPolicy(object):
    ...
    def allow_method(self, verb, resource, conditions=None):
        """ Allows access to the given HTTP verb and resource path """
        self._add_method("allow", verb, resource, conditions)

    def deny_method(self, verb, resource, conditions=None):
        """ Denies access to the given HTTP verb and resource path """
        self._add_method("deny", verb, resource, conditions)

    def build(self):
        """ Returns the final policy document as a dictionary """
        policy = {}
        policy['Version'] = self.version
        policy['Statement'] = []

        for method in self.allowMethods:
            statement = {}
            statement['Effect'] = "Allow"
            statement['Action'] = "execute-api:Invoke"
            statement['Resource'] = method['resourceArn']
            policy['Statement'].append(statement)

        for method in self.denyMethods:
            statement = {}
